...
@section('content')
<p>Apartado de editar proyectos.</p>
<div class="card">
    <div class="card-body">
        @if ($errors->any())
            ...
            <div class="row">
                <div class="col-6">
                    <div class="form-group">
                        ...
                        <input type="text" name="nombre" value="{{ $proyecto->nombre }}" class="form-control" id="nombre">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="id_grupo" class="selectpicker form-group" data-live-search="true" data-actions-box="true" title="Seleccione el grupo" data-width="100%">
                            @foreach($grupo as $key => $value)
                                <option value="{{ $key }}" {{ isset($proyecto->id_grupo) && $key == $proyecto->id_grupo ? 'selected' : '' }}>{{ $value }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="descripcion" class="form-control"  name="descripcion" rows="3">{{ $proyecto->descripcion }}</textarea>
                    </div>

                    <div class="form-group">
                        <label for="impacto">Impacto</label>
                        <textarea id="impacto" class="form-control"  name="impacto" rows="3">{{ $proyecto->impacto }}</textarea>
                    </div>

                    <div class="form-group">
                        <label for="objetivo">Objetivo</label>
                        <textarea id="objetivo" class="form-control"  name="objetivo" rows="3">{{ $proyecto->objetivo }}</textarea>
                    </div>

                    <div class="form-group">
                        <label for="id_categoria">Categoria</label>
                        <select name="id_categoria" class="selectpicker form-group" data-live-search="true" data-actions-box="true" title="Seleccione una categoria" data-width="100%">
                            @foreach($categoria as $key => $value)
                                <option value="{{ $key }}" {{ isset($proyecto->id_categoria) && $key == $proyecto->id_categoria ? 'selected' : '' }}>{{ $value }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="metodologia" class="selectpicker form-group" data-live-search="true" data-actions-box="true" title="Seleccione la metodologia" data-width="100%">
                            <option value="Cascada" {{ $proyecto->metodologia == 'Cascada' ? 'selected' : '' }}>Cascada </option>
                            <option value="Prototipos" {{ $proyecto->metodologia == 'Prototipos' ? 'selected' : '' }}>Prototipos </option>
                            <option value="Espiral" {{ $proyecto->metodologia == 'Espiral' ? 'selected' : '' }}>Espiral </option>
                            <option value="Etapas" {{ $proyecto->metodologia == 'Etapas' ? 'selected' : '' }}>Etapas </option>
                            <option value="RAD" {{ $proyecto->metodologia == 'RAD' ? 'selected' : '' }}>RAD </option>
                            <option value="Kanban" {{ $proyecto->metodologia == 'Kanban' ? 'selected' : '' }}>Kanban </option>
                            <option value="SCRUM" {{ $proyecto->metodologia == 'SCRUM' ? 'selected' : '' }}>SCRUM </option>
                            <option value="N/A" {{ $proyecto->metodologia == 'N/A' ? 'selected' : '' }}>N/A </option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="patente" class="form-control">
                            <option value="Si" {{ $proyecto->patente == 'Si' ? 'selected' : '' }}>SI</option>
                            <option value="No" {{ $proyecto->patente == 'No' ? 'selected' : '' }}>NO</option>
                        </select>
                    </div>
